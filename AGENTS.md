> **The block below is generated by Bram Setup** and rewritten on every upgrade. It is committed deliberately: left uncommitted it makes `git status` permanently modified, which trains the eye to ignore a signal that ought to mean something.
>
> One inaccuracy worth naming rather than hiding — it describes Bram's codex `developer_instructions` path, but this repo runs **Claude Code**. Real enforcement is in `.claude/settings.json`; the working conventions are in `.claude/bram-conventions.md`. Both are per-machine and untracked.
